...
class AktørController(private val aktørService: AktørService) {
    ...
    @GetMapping("v1")
    fun getAktørIdForPersonIdent(
        @RequestHeader(name = "Nav-Personident")
        personIdent: @NotNull String?
    ): ResponseEntity<Ressurs<Map<String, String>>> {
        return ResponseEntity.ok()
            .body(
                success(
                    mapOf("aktørId" to aktørService.getAktørId(personIdent)),
    ...
    fun getPersonIdentForAktørId(
        @RequestHeader(name = "Nav-Aktorid")
        aktørId: @NotNull String?
    ): ResponseEntity<Ressurs<Map<String, String>>> {
        return ResponseEntity.ok()
            .body(
                success(
                    mapOf("personIdent" to aktørService.getPersonIdent(AktørId(aktørId))),
