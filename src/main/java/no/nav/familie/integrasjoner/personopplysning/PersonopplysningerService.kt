...
class PersonopplysningerService @Autowired constructor(private val personSoapClient: PersonSoapClient,
    ...
    fun hentHistorikkFor(personIdent: String, fom: LocalDate, tom: LocalDate): PersonhistorikkInfo {
        ...
        request.aktoer = PersonIdent()
                .withIdent(NorskIdent().withIdent(personIdent))
        ...
        val response =
                personSoapClient.hentPersonhistorikkResponse(request)
        return oversetter.tilPersonhistorikkInfo(
                no.nav.familie.integrasjoner.personopplysning.domene.PersonIdent(personIdent), response)
    }
    ...
    fun hentPersoninfoFor(personIdent: String?): Personinfo {
        ...
        val response =
                personSoapClient.hentPersonResponse(request)
        return oversetter.tilPersoninfo(no.nav.familie.integrasjoner.personopplysning.domene.PersonIdent(personIdent),
                                        response)
    }
