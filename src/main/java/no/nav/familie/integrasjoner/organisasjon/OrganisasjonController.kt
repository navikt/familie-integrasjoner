...
class OrganisasjonController(
    ...
    ): Ressurs<Organisasjon> = success(data = organisasjonService.hentOrganisasjon(orgnr))

    @GetMapping(path = ["/{orgnr}/valider"], produces = [MediaType.APPLICATION_JSON_VALUE])
    fun validerOrganisasjon(
        @PathVariable orgnr: String,
    ): Ressurs<Boolean> = success(data = organisasjonService.validerOrganisasjon(orgnr))
}
